...
5. For long scripts or multi-chapter projects, create or update `生产资产/_内部/source-index.md` when it helps continuity. Treat it as internal evidence unless the user asks to see it.
...
11. Cross-batch asset reuse gate: before writing any formal `## 资产提示词`, build an Existing Asset Map from prior top-level `生产资产/连续投喂稿-*` asset headings, `生产资产/_内部/asset-bible.md`, `生产资产/_内部/source-index.md`, and `生产资产/_内部/image-manifest.json` when present. For every current-batch asset choose exactly one action: `reuse existing`, `rename confirmed anonymous-to-named`, `derived variant`, or `new`. Reused assets and confirmed renames must not emit a new full prompt in later batches; list them as `沿用第XX-XX章 图片N = 资产名` or bind the existing manifest path. Full prompts are only for genuinely new assets and source-justified derived variants. If a confirmed anonymous-to-named upgrade affects prior mother feeds or copy packs, update source-index and asset-bible first, then canonical mother feed -> audit -> copy packs; story wording may preserve reveal order, but asset bindings use one canonical identity or a recorded reveal-safe display name.
12. Draft shots internally: one video line = one visible action target, one main emotion or beat, one camera movement.
13. Formal production gate: if camera library or aspect ratio is not explicitly selected by the user or inherited from an existing audited feed, stop before drafting or writing the canonical feed or copy packs and ask in chat. Do not choose defaults, do not assume 小云雀, and do not assume 16:9.
14. Use this exact chat prompt only when production scope is already explicit and camera library or aspect ratio is missing: `正式生产参数缺失：请先选择运镜库（小云雀 / libtv）和画幅比例（9:16竖屏 / 16:9横屏 / 21:9电影）。收到选择前，我不会生成连续投喂稿或复制包。`
15. Use this exact chat prompt when camera library, aspect ratio, and production chapter count are all missing: `正式生产参数缺失：请先选择运镜库（小云雀 / libtv）、画幅比例（9:16竖屏 / 16:9横屏 / 21:9电影）和生产章数。检测到文本超过3章，建议先跑3章；你也可以指定 1-5 章、具体章节范围，或明确说全本分批。收到选择前，我不会生成连续投喂稿或复制包。`
16. Production scope gate: if an ambiguous formal production request may cover more than 3 chapters, stop before source-indexing, drafting, writing the canonical feed, or writing copy packs; ask the user how many chapters to produce and recommend 3 chapters. Do not write the canonical feed or copy packs before scope is chosen.
17. Use this exact chat prompt when formal production scope is missing: `生产范围缺失：检测到文本超过3章。建议先跑3章；你也可以指定 1-5 章、具体章节范围，或明确说全本分批。收到选择前，我不会生成连续投喂稿或复制包。`
18. Scope defaults after the gate: `默认` means chapters 1-3. Explicit 1 chapter delivers only chapter 1 in the canonical feed and copy packs. Explicit 1 chapter still reads chapters 1-4 when available for forward index. Full-book production batching defaults to 3 chapters per batch.
...
20. For any formal production standard, production mother feed, or formal copy-pack batch, confirm the aspect ratio before writing final video lines if the user has not already specified it: ask `画幅比例用哪个？默认 16:9。可选：9:16（竖屏）、16:9（横屏）、21:9（电影）。如果你说默认，我就按 16:9。` Only offer these three ratios. Do not offer `1:1` or `4:5`. If an existing audited feed already declares or clearly uses one selected ratio, keep that ratio unless the user asks to switch.
21. Emit only the final package:
   - `## 资产提示词`
   - `## 视频投喂块`
22. In `## 视频投喂块`, start with the global `统一要求` line using the selected aspect ratio, then number lines continuously from `1` to the end. Do not create 15-second groups, `第N组`, group titles, group footers, or per-group pacing blocks.
23. Before delivery, check source fidelity, dialogue preservation, cross-batch asset reuse, selected camera-library tags, selected aspect ratio, angle-bracketed camera/storyboard marking, continuous numbering, and multi-chapter coverage.
24. For formal production batches, run `copy-packager` after source index, asset bible, faithful feed, and feed audit exist; keep copy packs in a separate `生产资产` artifact.
...
- Keep the index under internal production assets, normally `生产资产/_内部/source-index.md` or a project-specific equivalent inside `生产资产/_内部/`. Legacy visible-path wording such as `生产资产/source-index.md` means the same production-asset family, but formal internal evidence should stay under `_内部`. It is a private working aid unless the user asks to see it.
...
- One line uses one camera tag from the selected library, written as a marked storyboard/camera term in angle brackets, such as `<固定镜头>` or `<镜头前推>`.
- Never write a bare camera-library term in production mother feeds or copy packs. Modifiers go after the bracketed raw tag, e.g. `<镜头前推>（压迫靠近）`.
- If 小云雀 and libtv contain the same raw tag, still write the selected raw tag as `<第一人称>` or `<第一视角>`; do not add a library prefix inside the video line.
...
| Video feed starts with a group title instead of the global requirement | Start `## 视频投喂块` with the selected-ratio `统一要求` line, then `1...2...3...` |
...
| Non-library or unmarked camera word appears | Replace with one exact tag from the selected camera library and wrap it in angle brackets, e.g. `<镜头前推>` |
...
| Later batch re-emits a full prompt for an existing asset | Build the Existing Asset Map first; write `沿用第XX-XX章 图片N = 资产名` or bind the manifest path instead of a new `GPT-image` prompt |
| Confirmed anonymous role becomes a separate named asset in a later batch | Record the source-index reconciliation, migrate the asset-bible binding, update affected mother feeds, then regenerate audit and copy packs |
