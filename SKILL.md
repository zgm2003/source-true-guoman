...
13. Emit only the final package:
   - `## 资产提示词`
   - `## 视频投喂块`
14. In `## 视频投喂块`, start with the global `统一要求` line, then number lines continuously from `1` to the end. Do not create 15-second groups, `第N组`, group titles, group footers, or per-group pacing blocks.
15. Before delivery, check source fidelity, dialogue preservation, asset reuse, selected camera-library tags, angle-bracketed camera/storyboard marking, continuous numbering, and multi-chapter coverage.
16. For formal production batches, run `copy-packager` after source index, asset bible, faithful feed, and feed audit exist; keep copy packs in a separate `生产资产` artifact.
